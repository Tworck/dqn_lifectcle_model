...
class Market(object):
    ...
    def step(self, action):
        ...
        if self.t < self.time_periods:
            ...
            # Taylor approximate the log utility for computational efficiency
    ...
    def generate_price_series(self):
        # trajectories x discretisations
        I = self.episodes
        M = self.time_periods
    ...
    # #*------ Do like this, its more smart
    # #*------ In terms of returns
    #   z = np.random.standard_normal((M,I))
    #   #df = 10
    #   #z = np.random.standard_t(df,I)
    #   S = np.exp((mu-0.5*sigma**2)*dt + sigma*math.sqrt(dt)*z)
    #   size = S.shape[1]
    #   B = np.ones(size)*np.exp(r*dt)

    #   # Make them absolute values again
    #   S = np.cumproduct(S)*S0
    #   B = np.cumproduct(S)*B0
